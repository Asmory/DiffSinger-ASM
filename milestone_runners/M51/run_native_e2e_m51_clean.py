...
def make_inputs(a, m30, model: dict, conf: dict):
    ...
    T = int(dur.sum())
    if T != 48:
        raise RuntimeError(f'M51 fixed-shape bundle expects T=48, got {T}')
...
def acoustic_cmd(a, out: Path):
    return [str(a.acoustic_cli), 'infer', str(a.packed_acoustic),
...
def prepare(a):
    ...
    print('M51 prepare: native acoustic reference')
    ...
    print('M51 prepare: ORT vocoder golden ONCE (outside all timed E2E runs)')
    ...
    gold = m30.flat_wave(sess.run([wn], feed)[0])
    gold_ms = (time.perf_counter() - t0) * 1000.0
    gold.astype(np.float32).tofile(a.fixture/'golden_wave.f32')
    meta = {
        'T': T, 'M': M, 'sample_rate': sr, 'hop_size': hop, 'samples': int(gold.size),
    ...
    print(f"M51 PREPARED fixture={a.fixture} audio={meta['audio_ms']:.3f} ms ORT_golden_once={gold_ms:.3f} ms")
...
def run_once(a):
    ...
    print('M51 CLEAN PATH: native acoustic -> native vocoder (NO ORT between them)')
    txt = run_capture(acoustic_cmd(a, melp))
    acoustic_ms = parse_ms(txt, r'time=([0-9.]+) ms')

    ref = np.fromfile(a.fixture/'reference_mel.f32', np.float32)
    got = np.fromfile(melp, np.float32)
    if ref.shape != got.shape:
        raise SystemExit(f'M51 acoustic shape mismatch {ref.shape} vs {got.shape}')
    bitdiff = int(np.count_nonzero(ref.view(np.uint32) != got.view(np.uint32)))
    max_abs = float(np.abs(ref-got).max(initial=0))
    print(f'M51 acoustic determinism: max_abs={max_abs:.9g} bitdiff={bitdiff}')
    if bitdiff:
        raise SystemExit('M51 acoustic output is not bit-exact to prepared reference; golden reuse invalid')

    env = os.environ.copy()
    env.update(DSASM_VNNI='k11', DSASM_GOLDEN_COS='0.999', DSASM_GOLDEN_SNR='25')
    wave = a.work/'native_wave.f32'
    vcmd = [str(a.vocoder_cli), 'infer', str(a.vocoder_bundle), '--mel', str(melp),
            '--f0', str(a.fixture/'f0.f32'), '--out', str(wave),
            '--golden', str(a.fixture/'golden_wave.f32'), '--workers', str(a.workers),
            '--rounds', str(a.rounds)]
    if a.profile:
        vcmd.append('--profile')
    vtxt = run_capture(vcmd, env=env)
    voc_ms = parse_ms(vtxt, r'median=([0-9.]+) ms')
    qual = parse_quality(vtxt)
    total = acoustic_ms + voc_ms
    audio_ms = float(meta['audio_ms'])
    rtf = total / audio_ms
    print(f'M51 CLEAN E2E: acoustic={acoustic_ms:.3f} vocoder={voc_ms:.3f} total={total:.3f} ms audio={audio_ms:.3f} RTF={rtf:.3f} speed={audio_ms/total:.3f}x')
    print(f'  realtime RTF<1: {"CROSSED" if rtf < 1 else "NOT YET"}; engineering RTF<=0.8: {"CROSSED" if rtf <= .8 else "NOT YET"}')
    report = {'acoustic_ms': acoustic_ms, 'vocoder_ms': voc_ms, 'total_ms': total,
              'audio_ms': audio_ms, 'rtf': rtf, 'quality': qual, 'rounds': a.rounds,
              'acoustic_bitdiff': bitdiff, 'acoustic_max_abs': max_abs,
              'clean_path_no_ort_between': True}
    (a.work/'m51_clean_e2e.json').write_text(json.dumps(report, indent=2) + '\n')
...
def main():
    ...
    ap.add_argument('--packed-acoustic', type=Path, required=True)
    ap.add_argument('--acoustic-cli', type=Path, required=True)
    ap.add_argument('--vocoder-bundle', type=Path, required=True)
    ap.add_argument('--vocoder-cli', type=Path, required=True)
    ap.add_argument('--vocoder-onnx', type=Path, required=True)
    ap.add_argument('--speaker-emb', type=Path, required=True)
    ap.add_argument('--fixture', type=Path, required=True)
    ap.add_argument('--work', type=Path, default=Path('build/m51/run'))
