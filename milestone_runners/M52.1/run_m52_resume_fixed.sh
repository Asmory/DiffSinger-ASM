...
printf '
========== M52.1 RESUME / TOPOLOGY MATRIX HOTFIX ==========
'
...
if [[ ! -f build/m52/topology.json ]]; then
...
[[ -f "$FIXTURE/meta.json" && -f "$FIXTURE/golden_wave.f32" ]] || {
  echo 'ERROR: missing M52 fixture; run the fixed all-in-one script instead.'
  exit 2
}
# Previous run already completed one-time prefault before the shell bug.
# Preserve the observed startup-prefault cost for final reporting unless overridden.
STARTUP_MS="${M52_STARTUP_MS:-130}"
echo "M52.1 resume: reusing prepared fixture and prior prefault; recorded startup_prefault=${STARTUP_MS} ms"
