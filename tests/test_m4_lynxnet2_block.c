...
static void init_block(BlockWeights *q, size_t T, size_t D, size_t H) {
    q->T=T;q->D=D;q->H=H;
    q->gamma=malloc(D*4);q->beta=malloc(D*4);q->dw_w=malloc(D*31*4);q->dw_b=malloc(D*4);
    ...
    pack_glu4(q->w1l,q->w1g,q->p1,H,D); pack_glu4(q->w2l,q->w2g,q->p2,H,H); pack_linear4(q->w3,q->p3,D,H);
}
static void free_block(BlockWeights *q){
    free(q->gamma);free(q->beta);free(q->dw_w);free(q->dw_b);free(q->w1l);free(q->w1g);free(q->b1l);free(q->b1g);
...
static void block_asm(const float *x, float *y, const BlockWeights *q,
...
static int test_block(size_t T,size_t D,size_t H,int bench) {
    ...
    float *x=malloc(T*D*4),*ref=malloc(T*D*4),*got=malloc(T*D*4);
    float *ln=malloc(T*D*4),*xp=malloc(D*(T+30)*4),*dwct=malloc(D*T*4),*dwtc=malloc(T*D*4),*h1=malloc(T*H*4),*h2=malloc(T*H*4),*proj=malloc(T*D*4);
    for(size_t i=0;i<T*D;i++)x[i]=frand_sym();
    block_ref(x,ref,&q); block_asm(x,got,&q,ln,xp,dwct,dwtc,h1,h2,proj);
    float d=max_abs_diff(ref,got,T*D);
    printf("LYNXNet2 block T=%zu D=%zu H=%zu: max_abs=%g %s\n",T,D,H,d,d<7e-4f?"OK":"FAIL");
    if(bench){
        int iters=20;
        double t0=now_sec();
        for(int i=0;i<iters;i++) block_asm(x,got,&q,ln,xp,dwct,dwtc,h1,h2,proj);
        double dt=(now_sec()-t0)/iters;
        double flops = 2.0*T*( (double)H*D*2.0 + (double)H*H*2.0 + (double)D*H ) + 2.0*T*D*31.0;
        printf("  asm composed: %.3f ms/block, arithmetic %.2f GFLOP/s\n",dt*1e3,flops/dt/1e9);
    }
    int fail=d>=7e-4f;
    free(x);free(ref);free(got);free(ln);free(xp);free(dwct);free(dwtc);free(h1);free(h2);free(proj);free_block(&q);
    return fail;
}
...
int main(void){
    ...
    if(fail){fprintf(stderr,"M4 regression FAILED\n");return 1;}
    puts("M4 regression PASSED");
    return 0;
}
