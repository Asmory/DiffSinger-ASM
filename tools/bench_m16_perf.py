#!/usr/bin/env python3
"""Perf-friendly official-shape workload for M16 cache-policy combinations."""
...
def main():
    ...
    ap.add_argument('--lib',type=Path,default=Path('build/libdsasm_m16.so'))
    ap.add_argument('--iters',type=int,default=80)
    ap.add_argument('--owner',type=int,choices=[0,1],required=True)
    ap.add_argument('--kblock',type=int,choices=[0,512],required=True)
    args=ap.parse_args()
    ...
    q=v.make_case(1016,64,128,384,1024,1024,6,'atan'); net,keep=v.build_ffi(q); T=64; C=1024
    ...
    lib.ds_threadpool_set_indexed_linear(pool,0); lib.ds_threadpool_set_n_owner(pool,args.owner)
    lib.ds_threadpool_set_kblocked_atan(pool,1 if args.kblock else 0); assert lib.ds_threadpool_set_k_block(pool,512)==0
    ...
    tag=f'owner={args.owner} kblock={args.kblock}'
    print(f'M16 PERF {tag} workers={n} cpus={cpus} median={np.median(ts):.3f} ms mean={np.mean(ts):.3f} p90={np.percentile(ts,90):.3f} ms')
