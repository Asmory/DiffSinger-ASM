#!/usr/bin/env python3
"""M17 target A/B: corrected default is dynamic scheduling + Kblock=512."""
...
def prepare(lib):
    q=v.make_case(917,64,128,384,1024,1024,6,'atan')
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--lib',type=Path,default=Path('build/libdsasm_m17.so')); ap.add_argument('--reps',type=int,default=31); args=ap.parse_args()
    ...
    print(f'M17 corrected-cache-policy A/B: official T=64 C=1024 L=6 workers={n} cpus={cpus}')
    ...
    for name,o,k in configs:
        ...
        print(f'  parity {name:18s}: max_abs={d:g}'); assert d==0.0
    ...
    for name,_,_ in configs:
        xs=np.asarray(buckets[name]); print(f'  {name:18s}: median={np.median(xs):7.3f} ms p90={np.percentile(xs,90):7.3f}')
    base=float(np.median(buckets['dynamic full-K'])); promoted=float(np.median(buckets['M17 dynamic kb512']))
    best=min(buckets,key=lambda k:float(np.median(buckets[k]))); bv=float(np.median(buckets[best]))
    print(f'  M17 vs full-K baseline: {base/promoted:.3f}x')
