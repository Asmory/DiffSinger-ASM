...
class Extras(C.Structure):
    _fields_=[('num_languages',C.c_uint32),('flags',C.c_uint32),('language_embedding',PF),('language_token_mask',PF),
              ...
              ('speed_weight',PF),('speed_bias',PF),('stretch_table',PF),('breath_scale',C.c_float),('voicing_scale',C.c_float),
...
def main():
    ...
    ex=Extras(nlang,LANG|BREATH|VOICE|TENSION|KEY|SPEED|SPK|LANGMASK,pf(lang_emb),pf(lang_mask),pf(bw),pf(bb),pf(vw),pf(vb),pf(tw),pf(tb),pf(kw),pf(kb),pf(sw),pf(sb),PF(),bs,vs,ts,gmin,gmax,gpre,ks,smin,smax,ss)
